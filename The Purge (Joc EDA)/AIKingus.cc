...
struct PLAYER_NAME : public Player {
  ...
  struct queueCell {
    ...
    int dist;       //steps away from the player
...
//BUILDERS' FUNCTIONS---------------------------------------------------------------------------------------------------------
  ...
  pair<Dir,Dir> perpendicular_dirs(const Dir d) {
    if (d == Up or Down) return make_pair(Right, Left);
  ...
  void avoid_warriors(const int& id) {
    ...
    if (bfs_next_step(p, enemy_step, 9) != -1) {  //If warriors active on the map, escape from them
      ...
      if (pos_ok(p+escape_step)) {  //if builder can go the exact opposite direction, go for it
        move(id, escape_step);
      } else {        //If not, he'll have to escape perpendicularly
        pair<Dir,Dir> esc_steps2 = perpendicular_dirs(escape_step);
        if (pos_ok(p+esc_steps2.first)) { 
          move(id, esc_steps2.first);
        } else if (pos_ok(p+esc_steps2.second)) { 
...
//WARRIORS' FUNCTIONS-------------------------------------------------------------------------------------------------------------
  ...
  void no_bazooka_hunt(const int& id) {
    ...
    if (citizen(id).weapon == Gun) {
      cerr << "I'm Mr. Gun " << id << " and closest enemy is " << bfs_next_step(p, kill_step, 2) << " far away" << endl;
      if (bfs_next_step(p, kill_step, 2) != -1) {  //If weaker enemy on the map, go kill him
        ...
        cerr << "I'm Mr. Hammer " << id << " and closest enemy is " << bfs_next_step(p, kill_step, 3) << " far away" << endl;
  ...
  void enemy_hunt(const int& id) {
    Pos p = citizen(id).pos;
    Dir kill_step;
    cerr << "I'm Mr. Bazooka " << id << " and closest enemy is " << bfs_next_step(p, kill_step, 1) << " far away" << endl;
