...
struct PLAYER_NAME : public Player {
  ...
  //Number of maxed barricades (maxed barricade = barricade with 280 resistance points / Max RP is 320 but we'll keep it 280 as the max)
...
//MOVEMENT BY OBJECTIVES FUNCTION (Money, Food, Weapons...)---------------------------------------------------------------------------
  ...
  bool bfs_closer_than_any_warrior(int idx, const Pos& orig) {
  ...
  //Parameter aim values: 1 -> get kills with Bazooka                    (search for less than Bazooka Enemy)
  ...
  //                     10 -> kill Gun/Hammer enemy Warriors            (s.f. less than Bazooka Enemy Warrior)
  //                     11 -> kill Hammer enemy Warriors                (s.f. less than Gun Enemy Warrior)
      ...
      if (pos_ok(ps)) {
        ...
        case 3: //HAMMER KILLS----------------------------
        {
          ...
          for (int k = 0; k < 4; ++k) {
            ...
            if (pos_ok(adj) and cell(adj).type == Street and (cell(adj).resistance <= 40 or cell(adj).b_owner == me() or cell(adj).b_owner == -1) and not is_ally(cell(adj).id) and (cell(adj).id == -1 or citizen(cell(adj).id).weapon == NoWeapon) and not visited[adj.i][adj.j]) {
              visited[adj.i][adj.j] = true;
              queueCell Adjacell = {adj, curr.dist+1, curr.first_step};
              Q.push(Adjacell);
            }
...
//WARRIORS' FUNCTIONS-------------------------------------------------------------------------------------------------------------
  ...
  bool stronger_enemy_there(int id, Pos enemy_ps) {
  ...
  void avoid_stronger_warriors(int id, const Dir enemy_dir) {
    ...
    if (pos_ok(p+escape_step) and cell(p+escape_step).type == Street and (cell(p+escape_step).b_owner == me() or not stronger_enemy_there(id, p+escape_step))) {  //if warrior can go the exact opposite direction, go for it
      move(id, escape_step);
    } else {        //If not, escape perpendicularly
      pair<Dir,Dir> esc_steps2 = perpendicular_dirs(escape_step);
      if (pos_ok(p+esc_steps2.first) and cell(p+esc_steps2.first).type == Street and (cell(p+esc_steps2.first).b_owner == me() or not stronger_enemy_there(id, p+esc_steps2.first))) { 
        move(id, esc_steps2.first);
      } else if (pos_ok(p+esc_steps2.second) and cell(p+esc_steps2.second).type == Street and (cell(p+esc_steps2.second).b_owner == me() or not stronger_enemy_there(id, p+esc_steps2.second))) { 
        move(id, esc_steps2.second);
      }
    } 
  }
  ...
  void enemy_hunt(int id) {
    ...
    if (ekill_dist == -1) {  //Highly improbable case: no enemies on the map
      search_money(id);
    } else if (wkill_dist != -1 and (wkill_dist < ekill_dist+2 or is_day())) {
        if (pos_ok(p+wkill_step)) {
          move(id, wkill_step);
        }
    } else {
