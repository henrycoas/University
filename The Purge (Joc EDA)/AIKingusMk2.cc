...
struct PLAYER_NAME : public Player {
  ...
  //"Strategic cell" defined as the one between two inaccessible positions
      ...
      if (pos_ok(ps)) {
          ...
          case 11: //FIND A STRATEGIC PLACE TO BUILD A BARRICADE
        {
          ...
          for (int k = 0; k < 4; ++k) {
            ...
            if (pos_ok(adj) and cell(adj).type == Street and cell(adj).b_owner == -1 and not visited[adj.i][adj.j] and cell(adj).is_empty()) {
              visited[adj.i][adj.j] = true;
              queueCell Adjacell = {adj, curr.dist+1, curr.first_step};
              Q.push(Adjacell);
            }
  ...
  void build_and_collect(const int& id) {
    ...
    if (cell(p).b_owner == -1 and (built_barricades < max_num_barricades() or maxed_barricades < built_barricades)) {
      ...
      if (not build_done and built_barricades < max_num_barricades()) {
        ...
        cerr << "Closest strategic point is " << build_dist << " so I will move " << build_step << endl;
...
//WARRIORS' FUNCTIONS-------------------------------------------------------------------------------------------------------------
  ...
  void find_stronger_weapon(const int& id) {
    ...
    if (citizen(id).weapon == Gun) {  //Warrior currently has a Gun, the only stronger weapon is the Bazooka
      ...
      if (((gun_dist == -1 and baz_dist != -1) or (baz_dist < gun_dist and baz_dist != -1)) and (is_day() or (is_night() and baz_dist < 8))) {
        if (pos_ok(p+baz_step)) {
          move(id, baz_step);
        }
      } else if (((baz_dist == -1 and gun_dist != -1) or (gun_dist < baz_dist and gun_dist != -1)) and (is_day() or (is_night() and gun_dist < 8))) {
  ...
  void search_food(const int& id) {
    ...
    if (bfs_next_step(p, food_step, 4) != -1) {  //If food available on the map, go there
      ...
      avoid_warriors(id); //Main goal is to heal so you can go killing again, so warriors won't exec this function many times
    }
  }
  ...
  void enemy_hunt(const int& id) {
    ...
    if (ekill_dist == -1) {  //Highly improbable case: no enemies on the map
      search_money(id);
    } else if ((wkill_dist != -1) and (wkill_dist < ekill_dist+3 or is_day())) {
